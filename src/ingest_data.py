...
class DataIngestor(ABC):       # inherit from ABC      
    ...
    def ingest(self, file_path: str) -> pd.DataFrame:   #def ingest(self, file_path: str) -> pd.DataFrame: This method signature specifies that it takes a string parameter (file_path) and returns a pandas DataFrame.
...
class ZipDataIngestor(DataIngestor):
    def ingest(self,file_path: str) -> pd.DataFrame:
        ...
        with zipfile.ZipFile(file_path,'r') as zip_file:
            zip_file.extractall("extracted_data")  #It extracts all contents of the ZIP file into a directory named extracted_data. If this directory does not exist, it will be created automatically.
        ...
        if len(csv_files) > 1:
            raise ValueError("More than one CSV file found in the extracted data. Please ensure there is only one or specify")
...
class DataIngestFactory:
    ...
    def get_data_ingetor(file_extension: str) -> DataIngestor:
        """Returns a DataIngestor instance based on the file extension."""
        if file_extension == ".zip":
            return ZipDataIngestor()
        else:
            raise ValueError(f"Unsupported file extension. Only .zip files are supported at this time not '{file_extension}'.")
